...
pub fn scale_image(image : &Image, new_width : u32, new_height : u32) -> anyhow::Result<Image> {
    ...
    let scaled_image = Image::new(&scaled_data, new_width, image.get_channels_per_pixel())?;
    Ok(scaled_image)
}
...
mod tests {
    ...
    fn reduce_width_only() -> anyhow::Result<()> {
        ...
        assert_eq!(scaled_image.get_pixel(0, 0), vec!(60, 175, 65));
        assert_eq!(scaled_image.get_pixel(1, 0), vec!(255, 10, 0));
        assert_eq!(scaled_image.get_pixel(0, 1), vec!(177, 105, 25));
        assert_eq!(scaled_image.get_pixel(1, 1), vec!(177, 105, 25));
        assert_eq!(scaled_image.get_pixel(0, 2), vec!(60, 175, 65));
        assert_eq!(scaled_image.get_pixel(1, 2), vec!(100, 200, 50));
        assert_eq!(scaled_image.get_pixel(0, 3), vec!(60, 175, 65));
        assert_eq!(scaled_image.get_pixel(1, 3), vec!(100, 200, 50));
        Ok(())
    }
